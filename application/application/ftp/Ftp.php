<?php
	...
	*/
	...
	class Ftp
	  {
		function Ftp($servidor = '', $usuario = '', $clave = '', $debug = false)
		  {
			$this->debug = $debug;
      if ($this->debug) echo "Entra Ftp<br>";
			$this->reset();
			if (trim($servidor) != '')
			  {
			...
			$this->conectar();
      if ($this->debug) echo "Sale Ftp<br>";
		  }
		  
		function reset()
		  {
      if ($this->debug) echo "Entra Reset<br>";
			$this->conexion = '';
			$this->archivo = '';
			$this->directorio = '';
			$this->registros = 0;
			$this->registro = array();
			$this->campo     = array();
      if ($this->debug) echo "Sale Reset<br>";
		  }
		function reset2()
		  {
      if ($this->debug) echo "Entra Reset<br>";
      ...
      if ($this->debug) echo "Sale Reset<br>";
		...
		function conectar()
		  {
      if ($this->debug) echo "Entra Conectar<br>";
      ...
      if ($this->debug) echo "Sale Conectar<br>";
		  }
		  
		function directorio( $directorio )
		  {
      if ($this->debug) echo "Entra directorio<br>";
      ...
      if ($this->debug) echo "Sale directorio<br>";
		  }
    
    function ftp_fetch ($stream, $archivo)
      {
      if ($this->debug) echo "Entra ftp_fetch<br>";
      //ob_end_flush();
      ob_start();
      ...
      fclose($out);
      $data = ob_get_clean();
      if ($this->debug) echo "Sale ftp_fetch<br>";
      return $data;
      }
    ...
    function traerPlano ($archivo)
      ...
      if ($this->debug) echo "Entra traer<br>";
    ...
    function traer ($archivo)
      ...
      if ($this->debug) echo "Sale traer<br>";
      return true;
      }
