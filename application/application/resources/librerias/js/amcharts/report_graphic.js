 ...
 * @param {array} parametros: los parametros que se enviaran en la peticion
 * @param {string} boton_activacion: el id o la referencia del boton el cual invoco el metodo
 ...
 * @date 2015/01/05
 */
function run_graphics(funcion, url, capa_grafi, parametros, boton_activacion) {
  ...
  $.ajax({
    ...
    beforeSend: function(data) {
      $(boton_activacion).attr('disabled', true);
    },
    complete: function(data) {
      $(boton_activacion).attr('disabled', false);
    },
    success: function(data) {
      if (data[0].length > 0) {
        ...
        $(capa_grafica).html('<p class="lead text-danger text-center">No hay datos registrados.</p>');
    },
    error: function(data) {
