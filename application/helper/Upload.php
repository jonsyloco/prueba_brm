...
/**
 *  NOTA: este helper es una copia modificada de la libreria Upload de CODEIGNITER CI 
 */
...
class Upload {
  ...
  /**
   * Obtiene informacion sobre el archivo, 
   * se creo con el fin de hacer logica antes de subir el archivo deacuerdo a sus datos.
   * @param type $field: el campo que contiene el archivo. el campo de formulario
   * @return array: el array con algunos datos.
   */
  function info_archivo($field = 'userfile') {
    if (isset($_FILES[$field])) {
      $_file = $_FILES[$field];
    }

    $nombre_archivo = $this->_prep_filename($_file['name']);
    $tamano_archivo = $_file['size'];// en bytes
    $extension_archivo = $this->get_extension($nombre_archivo);
    $tamano_kilobytes = 0;

    //convierte el tamaño a kilobytes
    if ($tamano_archivo > 0) {
      $tamano_kilobytes = round($tamano_archivo / 1024, 2);
    }
    return array('nombre' => $nombre_archivo, 'tamano' => $tamano_archivo, 'tamano_k' => $tamano_kilobytes, 'extension' => $extension_archivo);
  }
  ...
  public function do_upload($field = 'userfile') {
    ...
    if (!isset($_file)) {
      $this->set_error('No ha seleccionado ningun archivo'); // TRADUU
      return FALSE;
    }
    ...
    if (!is_uploaded_file($_file['tmp_name'])) {
      ...
      switch ($error) {
        case UPLOAD_ERR_INI_SIZE:
          $this->set_error('El archivo excede el limite de carga');
          break;
        case UPLOAD_ERR_FORM_SIZE:
          $this->set_error('El archivo excede el limite del formulario');
          break;
        case UPLOAD_ERR_PARTIAL:
          $this->set_error('El archivo no fue cargado completamente');
          break;
        case UPLOAD_ERR_NO_FILE:
          $this->set_error('No ha seleccionado ningun archivo');
          break;
        case UPLOAD_ERR_NO_TMP_DIR:
          $this->set_error('No se cargo ningun directorio temporal');
          break;
        case UPLOAD_ERR_CANT_WRITE:
          $this->set_error('No se pudo escribir el archivo');
          break;
        case UPLOAD_ERR_EXTENSION:
          $this->set_error('La carga del archivo fue detenida por su extension');
          break;
        default:
          $this->set_error('No ha seleccionado ningun archivo');
          break;
    ...
    if (!$this->is_allowed_filetype()) {
      $this->set_error('El tipo de archivo a subir no es valido');
      return FALSE;
    }
    ...
    if (!$this->is_allowed_filesize()) {
      $this->set_error('A sobrepasado el tamaño del archivo permitido');
      return FALSE;
    }
    ...
    if (!$this->is_allowed_dimensions()) {
      $this->set_error('La dimension del archivo no son validas');
      return FALSE;
    }
    ...
    if (!@copy($this->file_temp, $this->upload_path . $this->file_name)) {
      if (!@move_uploaded_file($this->file_temp, $this->upload_path . $this->file_name)) {
        $this->set_error('La carpeta de destino es invalida');
        return FALSE;
      }
    }
  ...
  public function set_filename($path, $filename) {
    ...
    if ($new_filename === '') {
      $this->set_error('El nombre del archivo cargo incorrectamente');
      return FALSE;
    } else {
      return $new_filename;
    }
  ...
  public function is_allowed_filetype($ignore_mime = FALSE) {
    ...
    if (empty($this->allowed_types) OR ! is_array($this->allowed_types)) {
      $this->set_error('No se cargo ningun tipo de archivo');
      return FALSE;
    }
  ...
  public function validate_upload_path() {
    if ($this->upload_path === '') {
      $this->set_error('No hay ninguna ruta para el archivo');
      return FALSE;
    }
    ...
    if (!$this->is_really_writable($this->upload_path)) {
      $this->set_error('No se puede escribir');
      return FALSE;
    }
  ...
  public function display_errors($open = '<p>', $close = '</p>') {
    ...
    return (count($this->error_msg) > 0) ? $open . implode($close . $open, $this->error_msg[0]) . $close : '';

    /* if(count($this->error_msg) > 0){
      echo 'hay mas de un error<br>';
