 ...
 */
class VentasClientes_controller extends ControladorBase {
    ...
    public function index()
    {
        ...
        $vista=  cargarView('VentasClientes');
    ...
    public function guardarVenta()
    {
        ...
        $respuesta=$miModelo->guardarVenta($cliente);

        if($respuesta == 0){            
            $idMaestro = $miModelo->obtenerId();
            if($idMaestro !=-1  && $idMaestro != null && $idMaestro!="") { //insertamos el detalle
                foreach ($prdcto as $item) {
                    $resulDetalle = $miModelo->guardarDetalleVenta($idMaestro,$item['nombre'],$item['cant'],$item['base'],$item['total']);
