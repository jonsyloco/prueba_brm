...
// Lock Screen/*****modificado por wagner
// tambien tenemos una parte del llamado en la ruta resource/js/vistaIdenx/indes.js***********/
(function( $ ) {
...
/*como ya inicializamos la primera vez que cargamos este pluggin. ya solo queda llamarlo*/
		initialize: function() {
		...
		build: function() {
			...
			this.$userEmail   = this.$lock.find( '#LockUserEmail' );
                        console.log("this.$userEmail "+this.$userEmail.val());
                        console.log("this.$userName "+this.$userName.val());
			return this;
		},
...
//la parte que nos intersa es el submit que emite los botones cuando se habre formulario
		formEvents: function( $form ) {
			...
			$form.on( 'submit', function( e ) {
				e.preventDefault();
                                   ...
                                   var pwd=$("#pwd").val();
                                    console.log("supuestamente hize submit");
                                    console.log("valor del campo, aqui vamos hacer AJAX para la peticion. y validacion "+pwd);
                                    //si el campo password ya desaparecio de pantalla entonces no pregunto la contraseña 
                                    if($("#pwd").length > 0){
                                        ...
                                        _self.hide();
                                            ...
                                            alertaNegra("la contraseña no es correcta");
                                        }
                                    }
									...
									'<form>',
										...
										'<div class="row">',
											...
											'<div class="col-xs-6 text-right">',
												'<button id="cerrarLock"  class="btn btn-primary" >Desbloquear</button>',
											'</div>',
										'</div>',
									'</form>',
								'</div>',
